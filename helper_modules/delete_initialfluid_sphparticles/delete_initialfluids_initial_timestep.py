...
def delete_initialfluids(sim, device, kernel, dt, mu, DX, rho0):
    """
    ...
    filterINITIALFLUID  = hoomd.filter.Type(['I']) # is two
    ...
    model = hoomd.sph.sphmodel.SinglePhaseFlow(kernel = Kernel,
                                               ...
                                               initialfluidgroup_filter = filterINITIALFLUID)
    ...
    with sim.state.cpu_local_snapshot as data:
        for i in range(len(data.particles.position)):
            ...
            if data.particles.typeid[i] == 2 and data.particles.mass[i] == -999:
                tags.append(data.particles.tag[i])
                # print(f'Rank: {device.communicator.rank} -> Delete Particle {data.particles.tag[i]}')
                deleted += 1
    ...
    print(f'Rank {device.communicator.rank}: {deleted} unnecessary initial fluid particles deleted.')
