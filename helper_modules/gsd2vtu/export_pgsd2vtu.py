...
def import_trajectory(PGSDfilename):
    """
    """
    # Input GSD file
    f = pgsd.fl.PGSDFile(name = PGSDfilename, mode = 'r+', application = "HOOMD-SPH", schema = "hoomd", schema_version = [1,0])
    # f = gsd.pygsd.GSDFile(open('log.gsd', 'rb'))

    # Parse GSD file into a trajectory object and return
    return pgsd.hoomd.HOOMDTrajectory(f)


def export_basic(PGSDfilename):
    """
    filename: str
    export basic fields

    """
    print(f'{os.path.basename(__file__)}: Export {PGSDfilename} to .vtu')

    t = import_trajectory(PGSDfilename = PGSDfilename)
    ...
    for snapshot in t:
        ...
        pname = PGSDfilename.replace('.gsd','')
