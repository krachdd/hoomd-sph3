...
enum
    {
    PGSD_WRITE_BUFFER_SIZE = 16 * 1024 * 1024
    };

/// Size of copy buffer
enum
    {
    PGSD_COPY_BUFFER_SIZE = 128 * 1024
    };
...
*/
inline static ssize_t pgsd_io_pwrite_retry(int fd, const void* buf, size_t count, int64_t offset)
    {
...
inline static int pgsd_expand_file_index(struct pgsd_handle* handle, size_t size_required, bool all)
    ...
    char* buf = malloc(PGSD_COPY_BUFFER_SIZE);
    ...
    while (total_bytes_written < old_index_bytes)
        {
        size_t bytes_to_copy = PGSD_COPY_BUFFER_SIZE;
        if (old_index_bytes - total_bytes_written < PGSD_COPY_BUFFER_SIZE)
            {
    ...
    pgsd_util_zero_memory(buf, PGSD_COPY_BUFFER_SIZE);

    size_t new_index_bytes = size_new * sizeof(struct pgsd_index_entry);
    while (total_bytes_written < new_index_bytes)
        {
        size_t bytes_to_copy = PGSD_COPY_BUFFER_SIZE;
        if (new_index_bytes - total_bytes_written < PGSD_COPY_BUFFER_SIZE)
            {
...
pgsd_initialize_file(MPI_File *fd, const char* application, const char* schema, uint32_t schema_version)
    ...
    //     }
    if(rank==0){
        MPI_File_write(fd, &namelist, sizeof(namelist), MPI_BYTE, MPI_STATUS_IGNORE);
    }
...
*/
inline static int pgsd_initialize_handle(struct pgsd_handle* handle)
    {
    // check if the file was created
    if (handle->fd == -1)
        {
    ...
    if (handle->open_flags != PGSD_OPEN_READONLY)
        ...
        retval = pgsd_byte_buffer_allocate(&handle->write_buffer, PGSD_WRITE_BUFFER_SIZE);
        if (retval != PGSD_SUCCESS)
            {
            return retval;
            }
...
int pgsd_create(const char* fname,
    ...
    // int retval = pgsd_initialize_file(fd, application, schema, schema_version);
...
int pgsd_create_and_open(struct pgsd_handle* handle,
    ...
    //                            O_RDWR | O_CREAT | O_TRUNC | extra_flags,
    //                            S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
    // int retval = pgsd_initialize_file(handle->fd, application, schema, schema_version);
...
int pgsd_truncate(struct pgsd_handle* handle)
    ...
    // retval = pgsd_initialize_file(handle->fd,
...
int pgsd_end_frame(struct pgsd_handle* handle)
    ...
    if (handle->frame_index.size > 0)
        ...
        if ((handle->file_index.size + handle->frame_index.size) > handle->file_index.reserved)
            {
            pgsd_expand_file_index(handle, handle->file_index.size + handle->frame_index.size);
            }
        ...
        size_t bytes_to_write = sizeof(struct pgsd_index_entry) * handle->frame_index.size;
        // ssize_t bytes_written
        //     = pgsd_io_pwrite_retry(handle->fd, handle->frame_index.data, bytes_to_write, write_pos);

        size_t bytes_written = sizeof(struct pgsd_index_entry)*entries_to_write;
        MPI_File_write_at(handle->fh, write_pos, data, sizeof(struct pgsd_index_entry)*entries_to_write, MPI_BYTE, MPI_STATUS_IGNORE);
...
int pgsd_read_chunk(struct pgsd_handle* handle, void* data, const struct pgsd_index_entry* chunk)
    {
    if (handle == NULL)
        {
        return PGSD_ERROR_INVALID_ARGUMENT;
