...
compute_filter_solid = hoomd.filter.Type(['S'])
compute_filter_wall  = hoomd.filter.Type(['W'])
...
solid_properties = hoomd.sph.compute.SolidProperties(compute_filter_solid)
...
logger[('custom', 'C_D')] = (lambda: ( solid_properties.total_drag_x/spf_properties.abs_velocity * viscosity) if ( spf_properties.abs_velocity > 0 ) else 0.0, 'scalar')
