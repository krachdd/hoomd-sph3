...
filtersuspended= hoomd.filter.Type(['O']) # is two
...
suspendedobjectintegrator = hoomd.sph.methods.SuspendedObjectIntegrator(filter=filtersuspended, densitymethod = densitymethod)


integrator.methods.append(velocityverlet)
integrator.methods.append(suspendedobjectintegrator)
integrator.forces.append(model)

compute_filter_all = hoomd.filter.All()
compute_filter_fluid = hoomd.filter.Type(['F'])
compute_filter_aggregate = hoomd.filter.Type(['O'])
spf_properties = hoomd.sph.compute.SinglePhaseFlowBasicProperties(compute_filter_fluid)
susf_properties = hoomd.sph.compute.SuspensionFlowBasicProperties(compute_filter_aggregate)
sim.operations.computes.append(spf_properties)
sim.operations.computes.append(susf_properties)
...
logger.add(susf_properties, quantities=['angularvel_x', 'angularvel_y', 'angularvel_z', 'translationvel_x', 'translationvel_y', 'translationvel_z'])
