...
"""Write PGSD files storing simulation trajectories and logging data."""
...
# Track open pgsd writers to flush at exit.
_open_pgsd_writers = []


def _flush_open_pgsd_writers():
    """Flush all open pgsd writers at exit."""
    for weak_writer in _open_pgsd_writers:
        writer = weak_writer()
        if writer is not None:
            writer.flush()


atexit.register(_flush_open_pgsd_writers)
...
def _finalize_pgsd(weak_writer, cpp_obj):
    """Finalize a PGSD writer."""
    _open_pgsd_writers.remove(weak_writer)
    cpp_obj.flush()


class PGSD(Writer):
    r"""Write simulation trajectories in the PGSD format.
    ...
    `PGSD` writes the simulation trajectory to the specified file in the PGSD
    format. `PGSD` can store all particle, bond, angle, dihedral, improper,
    pair, and constraint data fields in every frame of the trajectory.  `PGSD`
    ...
    over time. `PGSD` can also store arbitrary, scalar, string, and array
    ...
    +------------------+---------------------------------------------+
    | ``'xb'``         | Create a PGSD file exclusively.              |
    ...
    To reduce file size, `PGSD` does not write properties that are set to
    ...
    `PGSD` writes all non-default fields to frame 0 in the file.

    To further reduce file sizes, `PGSD` allows the user to select which specific
    fields will be considered for writing to frame 1+ in the `dynamic` list.
    When reading a PGSD file, the data in frame 0 is read when a quantity is
    ...
    * ``'topology'``
    ...
    ``'attribute'``), `PGSD` makes all the category member's fields dynamic.

    Warning:
        `PGSD` buffers writes in memory. Abnormal exits (e.g. ``kill``,
    ...
    See Also:
        See the `GSD documentation <https://gsd.readthedocs.io/>`__, `PGSD HOOMD
        ...
        information on PGSD files.

    Note:
        When you use ``filter`` to select a subset of the whole system, `PGSD`
        writes only the selected particles in ascending tag order and does
        **not** write out **topology**.

    Tip:
        All logged data fields must be present in the first frame in the pgsd
    ...
    def write(state, filename, filter=All(), mode='wb', logger=None):
        """Write the given simulation state out to a PGSD file.
    ...
    def logger(self, logger):
        if logger is not None and isinstance(logger, Logger):
            logger = _PGSDLogWriter(logger)
        else:
            raise ValueError("PGSD.logger can only be set with a Logger.")
        if self._attached:
    ...
    def flush(self):
        ...
        if not self._attached:
            raise RuntimeError("The PGSD file is unavailable until the"
...
def _iterable_is_incomplete(iterable):
    ...
    key. Otherwise, the PGSD backend will fail in its conversion to NumPy arrays.
    """
...
class _PGSDLogWriter:
        ...
        for key, value in _dict_flatten(self.logger.log()).items():
            ...
            if type_category not in self._skip_categories:
                if log_value is None:
                    ...
                    # per-{particle,bond,...} into the correct PGSD namespace
    ...
    def _log_convert_value(self, dict_, key, category, value):
        """Convert loggable types that cannot be directly stored by PGSD."""
