...
void RigidBodyIntegrator::integrateStepOne(uint64_t timestep)
    ...
    // Evaluate variant expressions — hoomd::Variant::operator() returns a plain Scalar.
    // The time derivative is approximated by a forward finite difference over one timestep.
    Scalar transvel_x    = (*m_transvel_x)(timestep);
    Scalar transvel_y    = (*m_transvel_y)(timestep);
    Scalar transvel_z    = (*m_transvel_z)(timestep);
    Scalar rotatvel      = (*m_rotatvel)(timestep);
    Scalar transvel_x_dt = ((*m_transvel_x)(timestep + 1) - transvel_x) / m_deltaT;
    Scalar transvel_y_dt = ((*m_transvel_y)(timestep + 1) - transvel_y) / m_deltaT;
    Scalar transvel_z_dt = ((*m_transvel_z)(timestep + 1) - transvel_z) / m_deltaT;
    Scalar rotatvel_dt   = ((*m_rotatvel)(timestep + 1) - rotatvel)     / m_deltaT;
    ...
    for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
        ...
        // Update slave particle positions (first half-step)
    ...
    // Wrap positions back into the periodic box so that particles which
    // cross a boundary (e.g. co-moving walls) are re-imaged correctly.
    // This mirrors what VelocityVerletBasic does after its position update.
        {
...
void RigidBodyIntegrator::integrateStepTwo(uint64_t timestep)
    ...
    // perform the second half step of the integration
    for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
        {
        unsigned int j = m_group->getMemberIndex(group_idx);

        // Update slave particle positions (second half-step)
        h_pos.data[j].x += Scalar(1.0/2.0)*h_vel.data[j].x*m_deltaT;
        h_pos.data[j].y += Scalar(1.0/2.0)*h_vel.data[j].y*m_deltaT;
        h_pos.data[j].z += Scalar(1.0/2.0)*h_vel.data[j].z*m_deltaT;
        }
