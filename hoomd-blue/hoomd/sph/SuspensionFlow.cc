...
SuspensionFlow<KT_, SET_>::SuspensionFlow(std::shared_ptr<SystemDefinition> sysdef,
    ...
    : SinglePhaseFlow<KT_, SET_>(sysdef, skernel, equationofstate, nlist, fluidgroup, solidgroup, mdensitymethod, mviscositymethod), m_suspendedgroup(suspendedgroup)
      {
        this->m_exec_conf->msg->notice(5) << "Constructing SuspensionFlow" << std::endl;

        // m_suspendedgroup(suspendedgroup),
...
template<SmoothingKernelType KT_,StateEquationType SET_>
void SuspensionFlow<KT_, SET_>::setParams(Scalar mu, Scalar rhoS, Scalar f0)
    {
    this->m_mu   = mu;
    m_rhoS = rhoS;
    m_f0 = f0;
...
void SuspensionFlow<KT_, SET_>::update_ghost_aux2(uint64_t timestep)
    ...
    if (this->m_comm)
        ...
        flags[comm_flag::auxiliary2] = 1; // repulsive force
...
void SuspensionFlow<KT_, SET_>::compute_noslipsuspended(uint64_t timestep)
    {
    this->m_exec_conf->msg->notice(7) << "Computing SuspensionFlow::NoSlip NoPenetration for Solid Group" << std::endl;
 ...
 */
template<SmoothingKernelType KT_,StateEquationType SET_>
void SuspensionFlow<KT_, SET_>::compute_Centerofmasses(uint64_t timestep)
    {
    ...
    // Find max suspended type number
        ...
        unsigned int suspendedtype_size = 0;
            for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
                ...
                if( particle_typeID == m_suspendedtypes[i] )
                    ...
                    // std::cout << particle_typeID <<particle_typeID << std::endl;
                    // std::cout << m_suspendedtypes[i] << m_suspendedtypes[i] << std::endl;
                    }
                }
 ...
 */
// TODO: Complete overhaul necessary for spheroids ...
template<SmoothingKernelType KT_,StateEquationType SET_>
void SuspensionFlow<KT_, SET_>::compute_equivalentRadii(uint64_t timestep)
    {
    ...
    // Loop over all suspended bodies
    for (unsigned int i = 0; i < m_suspendedtypes.size(); i++)
        {
        // Access COM position
        Scalar3  pi;
        ...
        Scalar meanradi = 0.0;
        // Set minimum for radi
        Scalar maxradi = 0.000001;
        unsigned int containing_particles = 0;
        unsigned int typeID_j;

        // Loop over all suspended particles (detect those who belong to the subgroup)
        unsigned int group_size = m_suspendedgroup->getNumMembers();
        for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
            {
            // Read particle index
...
template<SmoothingKernelType KT_,StateEquationType SET_>
void SuspensionFlow<KT_, SET_>::compute_repulsiveForce(uint64_t timestep)
    {
    // Read-Handle to velocity mass and position array
    ArrayHandle<Scalar4> h_pos(this->m_pdata->getPositions(), access_location::host, access_mode::read);
    ...
    // Loop over all suspended bodies (Happens on all cores at the same time)
    for (unsigned int i = 0; i < m_suspendedtypes.size(); i++)
        {
        // Access COM position of suspended i
        Scalar3  pi;
        pi.x = m_centerofmasses[i].x;
        pi.y = m_centerofmasses[i].y;
        pi.z = m_centerofmasses[i].z;
        ...
        for (unsigned int j = 0; j < m_suspendedtypes.size(); j++)
            {   
            if (i == j)
                continue;
            
            // Access COM position of suspended k
            Scalar3  pj;
            ...
            if ((dist < 0.2*radi_j))
                ...
                F_rep[1] =+ temp1*dx.y;
                F_rep[2] =+ temp1*dx.z;
                if ((sqrt(F_rep[0]*F_rep[0]+F_rep[1]*F_rep[1]+F_rep[2]*F_rep[2]))>0.0000001 and timestep%10)
                    {
                    this->m_exec_conf->msg->notice(2) << "Found contact between suspended " << i << " and suspended " << j <<"!" << endl;
                    this->m_exec_conf->msg->notice(2) << "Contact force on suspended particle "<< i << ": Fx = " << F_rep[0] << " Fy = " << F_rep[1] << ": Fz = " << F_rep[2] << endl;
                    }
                }
        ...
        // if ((sqrt(F_total[0]*F_total[0]+F_total[1]*F_total[1]+F_total[2]*F_total[2]))>0.0 and timestep%100 )
        //     {
        //     this->m_exec_conf->msg->notice(2) << "Total contact force on solid particle "<< i << ": Fx = " << F_total[0] << " Fy = " << F_total[1] << ": Fz = " << F_total[2] << endl; 
        //     }

        // Loop over suspended particles
        unsigned int group_size = m_suspendedgroup->getNumMembers();
        for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
            {
            ...
            // suspended-suspended contact force
            // If type id of suspended particle matches id of suspended body, force is applied. Counterpart is applied when i gets to this id
            if (typeID_k == typeID_i)
                ...
                // TODO: Change this to closest distance from wall
                // bool foundcontact = false;
                // // Loop over all of the neighbors of this particle
                // myHead = h_head_list.data[i];
                // size = (unsigned int)h_n_neigh.data[k];
                ...
                //     bool issolid = checksolid(h_type_property_map.data, h_pos.data[g].w);
            ...
            } // end loop over suspended particles

        } // end loop over suspended bodies
...
void SuspensionFlow<KT_, SET_>::forcecomputation(uint64_t timestep)
    ...
    if ( this->m_density_method == DENSITYSUMMATION )
        this->m_exec_conf->msg->notice(7) << "Computing SuspensionFlow::Forces using SUMMATION approach " << this->m_density_method << endl;
    else if ( this->m_density_method == DENSITYCONTINUITY )
        this->m_exec_conf->msg->notice(7) << "Computing SuspensionFlow::Forces using CONTINUITY approach " << this->m_density_method << endl;
    ...
    // Add volumetric force (gravity)
    if ( m_compute_fluid_acceleration)
        {
...
template<SmoothingKernelType KT_,StateEquationType SET_>
void SuspensionFlow<KT_, SET_>::forcecomputationsuspended(uint64_t timestep)
    {
...
void SuspensionFlow<KT_, SET_>::computeForces(uint64_t timestep)
    ...
    // Compute center of mass of solid bodies
    compute_Centerofmasses(timestep);

    // Compute equivalent radi of solid bodies ============= >>> evtl als Vektor speichern (x,y,z)
    compute_equivalentRadii(timestep);

    // Compute repulsive forces
    compute_repulsiveForce(timestep);
    ...
    // This includes the computation of the density if 
    // DENSITYCONTINUITY method is used
    forcecomputationsuspended(timestep);
...
void export_SuspensionFlow(pybind11::module& m, std::string name)
{
    pybind11::class_<SuspensionFlow<KT_, SET_>, SPHBaseClass<KT_, SET_> , std::shared_ptr<SuspensionFlow<KT_, SET_>>>(m, name.c_str()) 
        ...
        .def("deactivateFluidAcceleration", &SuspensionFlow<KT_, SET_>::deactivateFluidAcceleration)
