...
class SuspensionFlowBasicProperties(Compute):
    ...
    def _attach_hook(self):
        if isinstance(self._simulation.device, hoomd.device.CPU):
            spfbasic_cls = _sph.ComputeSusFBasicProperties
        else:
            spfbasic_cls = _sph.ComputeSusFBasicPropertiesGPU
        group = self._simulation.state._get_group(self._filter)
        self._cpp_obj = spfbasic_cls(self._simulation.state._cpp_sys_def, group)
        # super()._attach()
    ...
    @log(requires_run=True)
    def angularvel_x(self):
        """Sum of Fluid Particle velocity in xdir """
        self._cpp_obj.compute(self._simulation.timestep)
        return self._cpp_obj.angularvel_x

    @log(requires_run=True)
    def angularvel_y(self):
        """Sum of Fluid Particle velocity in xdir """
        self._cpp_obj.compute(self._simulation.timestep)
        return self._cpp_obj.angularvel_y

    @log(requires_run=True)
    def angularvel_z(self):
        """Sum of Fluid Particle velocity in xdir """
        self._cpp_obj.compute(self._simulation.timestep)
        return self._cpp_obj.angularvel_z

    @log(requires_run=True)
    def translationvel_x(self):
        """Sum of Fluid Particle velocity in xdir """
        self._cpp_obj.compute(self._simulation.timestep)
        return self._cpp_obj.translationvel_x

    @log(requires_run=True)
    def translationvel_y(self):
        """Sum of Fluid Particle velocity in xdir """
        self._cpp_obj.compute(self._simulation.timestep)
        return self._cpp_obj.translationvel_y

    @log(requires_run=True)
    def translationvel_z(self):
        """Sum of Fluid Particle velocity in xdir """
        self._cpp_obj.compute(self._simulation.timestep)
