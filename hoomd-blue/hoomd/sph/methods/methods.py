...
class SolidAggregateIntegrator(Method):
    r"""
    ...
    def _attach_hook(self):
        ...
        # initialize the reflected c++ class
        if isinstance(sim.device, hoomd.device.CPU):
            self._cpp_obj = _sph.SolidAggregateIntegrator(sim.state._cpp_sys_def,
                                           sim.state._get_group(self.filter))
        else:
            self._cpp_obj = _sph.SolidAggregateIntegratorGPU(sim.state._cpp_sys_def,
                                              sim.state._get_group(self.filter))

        # TODO: Hier eventuell auch group, vermutlich nicht
        # group = self._simulation.state._get_group(self.filter)
        # cpp_sys_def = self._simulation.state._cpp_sys_def
