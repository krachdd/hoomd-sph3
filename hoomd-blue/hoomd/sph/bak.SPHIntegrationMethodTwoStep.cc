...
/*! \file SPHIntegrationMethodTwoStep.cc
...
*/
// void SPHIntegrationMethodTwoStep::setProfiler(std::shared_ptr<Profiler> prof)
//     {
//     m_prof = prof;
//     }
...
*/
bool SPHIntegrationMethodTwoStep::restartInfoTestValid(IntegratorVariables& v, std::string type, unsigned int nvariables)
    {
    bool good = true;
    if (v.type == "")
        good = false;
...
/*! \param query_group Group over which to count (translational) degrees of freedom.
    ...
    group assigned to the method. Hence, the base class SPHIntegrationMethodTwoStep will implement that counting.
    Derived classes can ovveride if needed.
*/
unsigned int SPHIntegrationMethodTwoStep::getNDOF(std::shared_ptr<ParticleGroup> query_group)
    {
...
void export_SPHIntegrationMethodTwoStep(pybind11::module& m)
    {
    pybind11::class_<SPHIntegrationMethodTwoStep, std::shared_ptr<SPHIntegrationMethodTwoStep>(m, "SPHIntegrationMethodTwoStep")
        .def(pybind11::init< std::shared_ptr<SystemDefinition>, std::shared_ptr<ParticleGroup> >())
        .def("validateGroup", &SPHIntegrationMethodTwoStep::validateGroup)
#ifdef ENABLE_MPI
        .def("setCommunicator", &SPHIntegrationMethodTwoStep::setCommunicator)
#endif
        ;
    }
